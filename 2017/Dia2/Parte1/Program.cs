...
namespace Parte1
{
    class Program
    {
        ...
        {
            List<int> sumaDiferencias = new List<int>();
            int resultado = 0;
            int[,] rompecabezas = new int[,] { { 737, 1866, 1565, 1452, 1908, 1874, 232, 1928, 201, 241, 922, 281, 1651, 1740, 1012, 1001 },
            ...
            for (int i = 0; i < rompecabezas.GetLength(0); i++)
            {
                int numeroMayor = 0, numeroMenor = 10000, diferencia = 0;

                for (int j = 0; j < rompecabezas.GetLength(1); j++)
                {
                    if (rompecabezas[i, j] > numeroMayor) numeroMayor = rompecabezas[i, j];
                    if (rompecabezas[i, j] < numeroMenor) numeroMenor = rompecabezas[i, j];
                }

                diferencia = numeroMayor - numeroMenor;

                sumaDiferencias.Add(diferencia);
            }

            foreach (var item in sumaDiferencias)
            {
                resultado += item;
            }
